...
export default class VRM2WB extends Device {
  ...
  commands = {
    ...
    setLevel: {
      type: events.SET_CHANGE,
      cmd: ([index, level]) => `rxalevel(${index})=${level}\r`
    },
    setFreq: {
      type: events.SET_CHANGE,
      cmd: ([index, freq]) => `rxfreq(${index})=${freq}\r`
    },
    setBattType: {
      type: events.SET_CHANGE,
      cmd: ([index, type]) => `txbatt(${index})=${type}\r`
    }
  };
  ...
  _startScan() {
    ...
    devices.forEach(device => {
      ...
      this._scanData = Object.assign(this._scanData, {
        //TODO: this structure can be combined with _deviceData
        [device.index]: {
          block: device.block,
  ...
  _getDevicesToScan() {
    ...
    if (devices.length === 0) {
      throw new Error("Scan Error: device must be connected first");
    }
    return devices.filter((item, index) => {
      //Compares each item's block value and filters diplicates
  ...
  _updateScanData( response ) {
    if ( !response.index ) {
      throw new Error(
        "updateScanData requires a reciever index to associate data with"
      );
    }
    ...
    if ( offset > scanLength ) {
      //Adjust offset for array length
    ...
    this.scanDataHandler(this.scanData); // Why does a callback happen here but not in updateDeviceData?
  }

  _fetchData() {
    this._deviceData.forEach(ch => {
      //query block for each channel
  ...
  _jobSuccessHandler(result, job) {
    ...
    switch (result.type) {
      ...
      case events.FREQUENCIES: {
        const dataArray = this._parseData(result.payload).map(
      ...
      case events.BATTERY_VOLTAGE: {
        const dataArray = this._parseData(result.payload).map(
          volt => parseFloat(volt) / 100
        );
        this._updateDeviceData({ prop: "voltage", dataArray });
        break;
      }
      case events.OUT_LEVEL: {
        const dataArray = this._parseData(result.payload).map(lvl =>
          parseInt(lvl)
        );
  ...
  _jobErrorHandler(error) {
    //Handle an error from the message qeue
    this._msgQueue.end();
    this.stop();
