...
export default class VRWB extends Device {
  ...
  commands = {
    ...
    setLevel: {
      ...
      cmd: ([index, level]) => `level(${index})=${level}\r`
    },
    setFreq: {
      type: events.SET_CHANGE,
      cmd: ([index, freq]) => `mhz(${index})=${freq.toFixed(1)}\r`
  ...
  _jobSuccessHandler(result, job) {
    ...
    switch (result.type) {
      ...
      case events.FREQUENCIES: {
        const dataArray = this._parseData(result.payload).map(freq =>
          parseFloat(freq)
        );
      ...
      case events.OUT_LEVEL: {
        const dataArray = this._parseData(result.payload).map(level =>
          parseInt(level)
        );
  ...
  _setChannelSettings({ index, level, batteryType, frequency }) {
    if ([index, level, batteryType, frequency].includes(undefined)) {
      throw new Error(
        "Data Error: missing required key in channel data object"
      );
    }
